...
describe('linkifyReportCitations', () => {
  ...
  // Since the chip fix, known-source anchors become numbered #cite links that
  // jump to the SAVED source chunk; the web URL lives in the Sources list. The
  // old [[n](webUrl)] form sent readers to the original page — frequently
  // paywalled or Cloudflare-gated for exactly the pages research proxied.
  it('rewrites inline anchors to numbered chunk-jump links', () => {
    const records = [rec('9b96b9AA', 'https://ex.com/a', 'Paper A')];
    const { text, cited } = linkifyReportCitations('Claim [d9b96b9.s1.p6].', records);
    expect(text).toBe('Claim [[1](#cite:d9b96b9.s1.p6)].');
    expect(cited).toHaveLength(1);
    expect(cited[0].url).toBe('https://ex.com/a');   // Sources list keeps the URL
  });

  it('reuses the same number for repeated anchors of one source, each keeping its own chunk anchor', () => {
    ...
    expect(text).toBe('[[1](#cite:d9b96b9.s1.p6)] x [[2](#cite:d412359.s2.p1)] y [[1](#cite:d9b96b9.s3.p0)].');
    expect(cited.map(c => c.title)).toEqual(['A', 'B']);
  });

  it('leaves unknown anchors untouched for the raw-chip fallback', () => {
    const records = [rec('9b96b9AA', 'https://ex.com/a_(1)', 'A')];
    const { text } = linkifyReportCitations('[d9b96b9.s1.p6] and [dZZZZ99.s1.p1].', records);
    expect(text).toBe('[[1](#cite:d9b96b9.s1.p6)] and [dZZZZ99.s1.p1].');
  });

  it('still emits a chunk-jump link when the source has no URL (saved doc is the only home)', () => {
    const records = [rec('9b96b9AA', '', 'A')];
    const { text } = linkifyReportCitations('[d9b96b9.s1.p6].', records);
    expect(text).toBe('[[1](#cite:d9b96b9.s1.p6)].');
  });

  it('falls back to the web URL (parens escaped) only when the record has no docId', () => {
    const records = [{ docId: '', url: 'https://ex.com/a_(1)', title: 'A', label: 'WEB' as const, tier: 'standard' as const }];
    // No docId → not in the short-id map → anchor stays raw (resolvable later).
    const { text } = linkifyReportCitations('[d9b96b9.s1.p6].', records);
    expect(text).toBe('[d9b96b9.s1.p6].');
  });
});
