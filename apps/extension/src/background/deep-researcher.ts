...
/**
 * Wall-clock budget for the GATHER loop. The run-level watchdog aborts the
 * whole job at 60 min (service-worker RESEARCH_MAX_WALL_MS) and the user gets
 * NOTHING — which is what a long run actually looked like in practice. Stopping
 * the gather at 35 min leaves room to synthesize, evaluate and save a report
 * from whatever was collected: a shorter report beats an aborted one.
 */
...
export function linkifyReportCitations(
  ...
  for (const r of records) {
    ...
    const prior = byShort.get(short);
    if (prior && prior.docId !== r.docId) {
      // Two different documents behind one 6-char short id: an anchor alone
      // cannot say which one it cites, and picking the first would NUMBER the
      // second doc's citations as the first doc's source — a silently WRONG
      // citation. Leave both raw; the chip resolver looks the anchor up in
      // the chunk store, which is exact.
      collided.add(short);
      continue;
    }
    if (!prior) byShort.set(short, r);
  }
  ...
  const text = synthesis.replace(re, (full, anchor: string, short: string) => {
    ...
    // so clicking a citation opened the original web page (frequently
    // paywalled or Cloudflare-gated for exactly the pages research had to
    // scrape through a reader proxy) instead of the SAVED source. The web
    // URL still ships in the Sources list entry n, so exported markdown
    // keeps a working external reference.
    if (rec.docId) return `[[${n}](#cite:${anchor})]`;
...
/**
 * Assemble the final report body: linkified citations + numbered Sources list.
 *
 * THE invariant this function owns — and the reason it is pure and exported:
 * for every inline citation [[n](#cite:anchor)], `ordered[n-1]` is the source
 * record of the document the anchor points into. Cited sources come first, in
 * citation order; uncited sources follow. A numbered marker that names the
 * wrong source is worse than no citation, so this is property-tested rather
 * than trusted.
 */
...
async function saveSynthesisReport(
  ...
  const { body } = assembleReportBody(synthesis, topic, sources);
...
- Use markdown tables wherever the evidence contains comparable or quantitative data — but NEVER for code (code in cells corrupts the table; use labelled fenced blocks instead).
...
async function scoreWebRefs(topic: string, refs: HarvestedRef[]): Promise<HarvestedRef[]> {
  ...
  try {
    // MUST go through sendToOffscreen, not chrome.runtime.sendMessage: that
    // client owns the mutex serializing ONNX work. A raw send jumps the queue
    // and can run the reranker concurrently with an embed batch — extra peak
    // heap on the isolate with the documented ~2.9GB ratchet. It also gets the
    // finite deadline + dead-document recreate for free.
...
/**
 * Revise ONLY the sections the auditor flagged, leaving every other section
 * byte-identical.
 *
 * Why this exists: reviseSynthesis rewrites the whole report and is told to
 * "expand and restructure". An LLM restructuring 3000 words drops most inline
 * [anchor] citations even though the prompt says to keep them — which is how
 * reports ended up long and link-free. It also truncated at 24k chars, so a
 * long report lost its tail entirely.
 *
 * Returns null when section-scoping doesn't apply (no headings, nothing
 * specific flagged, or everything flagged) so the caller can fall back to the
 * whole-report path.
 */
async function reviseFlaggedSections(
  topic: string,
  report: string,
  evaluation: EvaluationResult,
  sourceContext: string,
  ...
  const flaggedIdx = sections
    .map((sec, i) => ({ sec, i }))
    .filter(({ sec }) => evaluation.flaggedSections.some(f => sectionMatchesFlag(sec.heading, f)))
    .map(({ i }) => i);

  // Nothing specific, or the auditor is unhappy with everything — a targeted
  // rewrite buys nothing over the whole-report path in either case.
  if (flaggedIdx.length === 0 || flaggedIdx.length === sections.length) return null;

  onProgress(`[EVALUATING] Revising ${flaggedIdx.length} flagged section(s), keeping the rest verbatim…`);
  const next = [...sections];
  let changed = 0;
  ...
  for (const i of flaggedIdx) {
    ...
    const sys =
      `You are revising ONE SECTION of a research report on: "${topic}" after a quality audit.\n` +
      `Auditor's findings for the report:\n${evaluation.weaknesses.map(w => `- ${w}`).join('\n')}\n\n` +
      `Rewrite ONLY the section below, addressing the findings that apply to it using the source excerpts.\n` +
      `- Return the section body ONLY. Do NOT repeat the heading. Do NOT write any other section.\n` +
    ...
    const revised = await withKeepAlive(
      `[EVALUATING] Revising "${sec.heading.replace(/^#+\s*/, '').slice(0, 40)}"`,
      llmChatFn(sys, user),
      onProgress,
      30_000
    ).catch(() => '');

    const body = (revised || '').trim();
    if (body.length < 100) continue;                       // no usable output — keep original

    // The guard this whole change exists for: a rewrite that gutted the
    // evidence is worse than the section it replaced.
    if (!revisionKeptCitations(sec.body, body)) {
      onProgress(`[EVALUATING] ⚠ Revision of "${sec.heading.replace(/^#+\s*/, '').slice(0, 40)}" dropped citations (${countCitations(sec.body)} → ${countCitations(body)}) — keeping the original`);
      continue;
    }
    next[i] = { heading: sec.heading, body };
    changed++;
  }

  if (changed === 0) return null;
  return joinReportSections(preamble, next);
}
...
async function evaluateAndRefine(
  ...
  for (let pass = 1; pass <= MAX_REVISIONS; pass++) {
    ...
    // Prefer the section-scoped path: it rewrites only what the auditor named
    // and leaves the rest byte-identical, so citations elsewhere survive. Falls
    // back to the whole-report rewrite when scoping doesn't apply (no headings,
    // nothing specific flagged, or every section flagged).
    const citesBefore = countCitations(current);
    const scoped = await reviseFlaggedSections(topic, current, ev, sourceContext, llmChatFn, onProgress)
      .catch(() => null);
    let revised = stripModelBibliography(
      scoped ?? await reviseSynthesis(topic, current, ev, sourceContext, llmChatFn, onProgress).catch(() => current)
    );
    // Last-ditch guard on the whole-report path, which is the one that loses
    // citations wholesale. A revision that gutted the evidence is not an
    // improvement no matter what the rubric says about its prose.
    if (!scoped && citesBefore > 0 && !revisionKeptCitations(current, revised)) {
      onProgress(`[EVALUATING] ⚠ Full-report revision dropped citations (${citesBefore} → ${countCitations(revised)}) — keeping the previous draft`);
      revised = current;
    }
...
Return ONLY this JSON object:
{
  ...
  try {
    ...
    if (!parsed) {
      onProgress?.(`[STAGE ${stage}/${rounds}] ⚠ Reflect returned unparseable output (${res.length} chars) — outline not updated`);
      return null;
    }
    ...
    // A deadline abort and a provider error look identical to the caller
    // otherwise; say which so a recurring failure is diagnosable from the log.
    const why = e?.message === 'deadline' || e?.name === 'AbortError'
      ? 'hit the per-call deadline'
      : (e?.message || 'failed');
    onProgress?.(`[STAGE ${stage}/${rounds}] ⚠ Reflect ${why} — outline not updated`);
    return null;
  }
}
...
async function runDeeperResearch(
  ...
  for (let stage = 1; stage <= rounds; stage++) {
    ...
    // Out of gather time? Stop collecting and go synthesize what we have,
    // rather than letting the run-level watchdog abort everything at 60 min.
    // Only from stage 2 — stage 1 must always run or there is nothing to write.
    const gatherElapsed = Date.now() - gatherStartedAt;
    if (stage > 1 && gatherElapsed > GATHER_WALL_BUDGET_MS) {
      onProgress(`[STAGE ${stage}/${rounds}] [TIME BUDGET] ${Math.round(gatherElapsed / 60000)} min spent gathering — stopping here and writing the report from ${gatheredDocIds.size} source(s).`);
      crumb('research', 'gather budget exhausted', { stage, elapsedMin: Math.round(gatherElapsed / 60000) });
      break;
    }
    ...
    if (uniqueStageDocIds.length > 0) {
      ...
      if (brief) {
        ...
        if (r) {
          ...
          const novel = uniqueStageDocIds.filter(d => !gatheredDocIds.has(d)).length;
          onProgress(`[STAGE ${stage}/${rounds}] ✓ Outline: ${outline.sections.length} sections (${thin} still thin) · ${novel} new source(s) this stage`);

          // Graded early stop. The old condition was `thin === 0` — EVERY
          // section adequate — so one stubborn section kept all 8 stages
          // running, and stages 4-8 are roughly half the run. `rounds` was
          // raised 4 → 8 for report depth (fab4167); this is what makes that
          // affordable: a saturated topic stops early, a genuinely broad one
          // still gets the full budget.
          //
          // TWO independent signals must agree, so one noisy model judgment
          // can't end a run prematurely:
          //   • coverage — at most a quarter of sections still thin (tolerates
          //     the one section no source will ever satisfy), and
          //   • novelty — this stage indexed NOTHING the run hadn't already
          //     seen. A stage that found nothing new won't next round either.
          // Floored at stage 3 so a lucky early outline can't cut a run short.
          const thinRatio = outline.sections.length > 0 ? thin / outline.sections.length : 1;
          const saturated = stage >= 3 && thinRatio <= 0.25 && novel === 0;

          if (thin === 0 || saturated) {
            onProgress(thin === 0
              ? `[STAGE ${stage}/${rounds}] [DYNAMIC STOP] All outline sections are adequate or rich. Stopping gather loop early.`
              : `[STAGE ${stage}/${rounds}] [DYNAMIC STOP] Saturated: ${thin}/${outline.sections.length} sections thin and no new sources this stage. Stopping gather loop early.`);
          ...
          // The graded stop above lives inside the reflect-succeeded branch
          // because it needs the outline's section statuses. When reflect
          // fails there IS no outline — but novelty alone still decides this
          // case, and decides it more sharply: without reflect the next
          // stage re-runs the SAME sub-questions as queries, so it will
          // re-find the same sources. A stage that already found nothing new
          // is the start of a loop that cannot make progress, and without an
          // outline the sectioned synthesis can't use the extra stages
          // anyway.
          const novelWithoutReflect = uniqueStageDocIds.filter(d => !gatheredDocIds.has(d)).length;
          if (stage >= 3 && novelWithoutReflect === 0) {
            onProgress(`[STAGE ${stage}/${rounds}] [DYNAMIC STOP] No outline and no new sources this stage — further stages would repeat the same queries. Stopping gather loop early.`);
            stageBriefs[stage - 1] = brief;
            uniqueStageDocIds.forEach(d => gatheredDocIds.add(d));
            await updateJob({
              phase: 'gathering',
    ...
    // NOTE: no recycleOffscreenWorker() here. recreateOffscreen() below tears
    // down the whole offscreen document — including that inference worker — so
    // terminating and respawning it first only paid for an extra model warm-up
    // at every stage boundary and reclaimed nothing the recreate didn't.
    await recreateOffscreen().catch(() => {});
  }
