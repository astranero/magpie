 ...
 */
/** Currently UNUSED. Was called at every research stage boundary immediately
 *  before recreateOffscreen(), which tears down the whole document including
 *  this worker — so it only bought an extra model warm-up. Kept as a primitive
 *  for a caller that wants to reclaim the worker WITHOUT recreating the
 *  document; delete if none appears. */
