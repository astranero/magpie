 ...
 */
/**
 * Close a JSON string that was cut off mid-generation.
 *
 * The reflect call returns the whole outline — 4-8 sections with verbatim
 * [anchor] evidence notes — so it is the largest JSON of a run and the most
 * likely to hit a token ceiling or a mid-stream deadline abort. When that
 * happens the naive `lastIndexOf('}')` slice lands on an INNER brace and the
 * parse dies, discarding an outline that was 90% complete. Balancing the open
 * containers salvages every section that did finish.
 *
 * Returns null when the text isn't actually truncated (nothing to salvage).
 */
export function closeTruncatedJson(text: string): string | null {
  ...
  if (!inString && stack.length === 0) return null;   // complete already

  let out = text;
  if (inString) out += '"';                            // close the cut string
  // Drop a dangling `"key":` or trailing comma the cut left behind.
...
export function parseReflect(raw: string): ReflectResult | null {
  ...
  const text = end > start ? cleaned.slice(start, end + 1) : '';

  const attempt = (t: string): any => {
    if (!t) return null;
    try { return JSON.parse(t); } catch { /* try repairs */ }
  ...
  let parsed: any = attempt(text);
  if (!parsed) {
    // Truncated mid-generation: balance the open containers and retry against
    // everything from the first brace to the end (NOT the lastIndexOf slice,
    // which cuts at an inner brace on truncated input).
    const closed = closeTruncatedJson(cleaned.slice(start));
    if (closed) parsed = attempt(closed);
  }
