...
// Removed importImageFiles due to deprecation of local image analysis to save memory footprint.
  ...
  const send = async () => {
    ...
    if (matched) {
      if (!matched.query) {
        const history = messages[activeChatId] || [];
        if (history.length > 0) {
          setInput('');
          sendCommandOverStream('the above conversation', matched.command.label!, matched.command.systemPrompt!);
          return;
        }
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-background text-foreground overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative bg-background">
        ...
        {/* No gutter: a side panel is a WALL of the browser, not a card floating
            inside one. Each view owns its own padding, so content runs to the
            panel edge and the vertical rules line up with the window edge. */}
...
onOpenDocument={(docId, anchorId) => openDocById(docId, anchorId, 'chat')}
          ...
          {view === 'settings' && (
            ...
            <SettingsView 
              ...
              activateProviderModel={(provider: 'copilot' | 'byok', model: string) => {
                // One writer for "make this provider+model live" — the chat
                // header's onSelectModel and both Settings sections route
                // through the same trio write so they can never disagree.
                setCustomModel(model);
                if (provider === 'copilot') {
                  const base = copilotApiBase || customUrl;
                  setActiveProvider('copilot'); setCustomUrl(base); setCustomKey('__copilot_sso__');
                  chrome.storage.local.set({ activeProvider: 'copilot', customUrl: base, customKey: '__copilot_sso__', customModel: model, customModels: copilotModels });
                } else {
                  const url = byokUrl || customUrl;
                  const key = byokKey || customKey;
                  setActiveProvider('byok'); setCustomUrl(url); setCustomKey(key);
                  chrome.storage.local.set({ activeProvider: 'byok', customUrl: url, customKey: key, customModel: model, customModels: byokModels.length ? byokModels : customModels });
                }
              }}
