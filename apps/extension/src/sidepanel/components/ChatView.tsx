...
const MessageBody: React.FC<MessageBodyProps> = React.memo(({ text: rawText, compact, streaming, renderLive, resolveCitations, onOpenDocument, onOpenExternalLink }) => {
  ...
  const text = useMemo(
    () => fastPath ? rawText : normalizeLatexDelimiters(normalizeCitations(stripInvisibleMathOps(rawText))),
    [rawText, fastPath]
  );
  ...
  const anchorOrder = useMemo(() => {
    ...
    // Anchors already wrapped in #cite links (research reports are saved
    // pre-linkified by linkifyReportCitations) — pick those up too.
    const linked = /\(#cite:([a-z]\w{1,8}\.s\d+\.p\d+(?:\.\d+)?)\)/gi;
    while ((m = linked.exec(text)) !== null) {
      if (!order.includes(m[1])) order.push(m[1]);
    }
    return order;
  }, [text, fastPath]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      // #cite-carried anchors aren't raw markers; append them so the resolver
      // (which scans only [anchor] text) sees every anchor on screen.
...
/** ModelSelect footer slot: refreshes the active provider's catalog in place. */
const ModelRefreshFooter: React.FC<{ onRefresh?: () => Promise<void> }> = ({ onRefresh }) => {
  const [fetching, setFetching] = useState(false);
  if (!onRefresh) return null;
  const run = async () => {
    setFetching(true);
    try { await onRefresh(); } catch (e) { console.error(e); } finally { setFetching(false); }
  };
  return (
    <div className="border-t border-border/60 pt-1.5 mt-0.5 px-1.5 pb-1.5">
      <button
        type="button"
        onClick={run}
        disabled={fetching}
      ...
      >
        {fetching ? <><Loader2 size={11} className="animate-spin" /> Fetching…</> : <><Sparkles size={11} /> Refresh Models</>}
      </button>
    </div>
  );
};
...
export const ChatView: React.FC<ChatViewProps> = ({
  ...
  // Panel reopened from scratch: put the reader back where they were rather
  // than at the bottom. (The visibility-change case — returning from a cited
  // document or another tab, without a remount — is handled by the effect
  // below.) The flag suppresses the auto-scroll effect exactly once; that is
  // correct HERE because both effects do fire on mount, in order.
  const restoredScroll = useRef(false);
  useEffect(() => {
    if (scrollBox.current && shouldRestoreScroll(scrollPosRef?.current, scrollBox.current)) {
  ...
  // Coming back into view — from a cited source document, or from another tab.
  // The panel is hidden with display:none, which RESETS scrollTop to 0, so the
  // reader's place has to be put back explicitly or they land at the bottom of
  // a long transcript having lost where they were reading.
  //
  // scrollPosRef is kept current by the container's onScroll below. Reading
  // scrollHeight here forces the pending layout to flush, so the metrics are
  // valid even though the element only just stopped being display:none.
  //
  // NOTE: deliberately does NOT set restoredScroll. That flag suppresses the
  // messages-effect auto-scroll exactly once, and that effect does not re-run
  // on a visibility change — setting it here would leave it armed and eat the
  // next legitimate scroll-to-bottom when a reply arrives.
  ...
  useEffect(() => {
    ...
    if (box && shouldRestoreScroll(scrollPosRef?.current, box)) {
      box.scrollTop = scrollPosRef!.current!;
      return;
    }
    msgEnd.current?.scrollIntoView({ behavior: 'instant' });
  }, [isActive, scrollPosRef]);
        ...
        {modelEntries.length > 0 && (
          <div className="flex items-center justify-between px-1">
            <ModelSelect
              ...
              badge={e => e.meta === 'copilot' ? (
                <span className="text-[9px] font-mono uppercase tracking-wide px-1 rounded bg-primary/10 text-primary shrink-0">Copilot</span>
              ) : null}
              footer={<ModelRefreshFooter onRefresh={onRefreshModels} />}
              entries={modelEntries.map(e => ({ model: e.model, group: e.group, meta: e.provider }))}
              value={customModel}
              placeholder="Select model…"
              onSelect={e => onSelectModel?.({ provider: (e.meta as 'byok' | 'copilot') || 'byok', model: e.model })}
            />
