...
**Status: DONE — re-verified against the current codebase on 2026-07-23.**
Every suite verdict and both extractions below still hold: `lib/frontmatter.ts`
exports `splitFrontmatter`/`parseFrontmatterFields`, consumed by both
`DocumentView.tsx` and `chunker.ts` (`frontmatter-parse.test.ts` imports them,
not a local copy); `vector-store.ts` exports `gateRerankedChunks`, covered by
`rerank-gate.test.ts`; `quality-gate.test.ts` has the per-phrase isolation
block; `paper-rank.test.ts` has the velocity-term property assertion. Full
suite is green at 535/535 across 52 files (`npx vitest run`), up from 154/154
at audit time — growth from later feature work, not a regression. (Note:
`PLAN-v1-full.md`'s Phase A acceptance-criteria checkboxes for A1/A2 are still
unchecked there even though this doc and the header-level ✅ markers show the
work landed — that's a separate doc, left untouched here.)
