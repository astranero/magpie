...
| `documents` | `id` | Full markdown (`content`, incl. frontmatter), title, url, `capturedAt`, `wordCount`, optional `bibtex` |
...
visionModel`), `researchDepth`, `sourceQuality`, `contextTokens`, `s2ApiKey`,
`searchApiKeys` (Tavily/Brave/Serper), `mcpServers`, `customSkills`,
`autoLinkCaptures`, `includePageContext`, and the crash-safe **research job
checkpoint** (`ara-research-job`: plan, phase, logs, `active`,
`lastHeartbeatAt`, `resumeAttempts`).
