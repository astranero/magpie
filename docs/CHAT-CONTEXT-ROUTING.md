...
- **agentic** (default) — `agenticGather`: bounded tool loop (≤3 rounds) via
  `chatWithTools`. Repo/link tools (`read_file`/`read_link`/`search_web`) plus
  the PAGE tools: `read_section(heading)` (8k cap + truncation notice),
  `search_page(query)` (grep the whole page, matches + context — how the model
  finds an error beyond the inlined head), `read_lines(start, count)`. Tool
  results stream to `onStatus` so the user sees the enrichment live. **Throws
  on round-0 no-tool-calls so the caller falls back to semantic** (a non-tool
  model would otherwise return zero context).
- **semantic** — `selectSemantic`: explicit filename match → else rerank
  keyword-filtered paths; links: lexical (nav-synonym expanded) → else reranker
  picks the best link **by the question**, gated to non-meta asks and a 0.5
  confidence bar.
...
- **Enumeration** — `isEnumerationQuestion` ("what X are…", "list…", "other…")
  ...
  list is present. (`selectPageMarkdown` is gone — the head + section-index
  injection plus `read_section` covers this now.)

## Debug-page routing (`lib/log-highlights.ts`)

A page that IS failure output gets the debugger prompt ("find the ROOT CAUSE,
name the exact error / file / line") instead of the gentle reading prompt, and
build logs additionally get `extractLogHighlights` — the error/warning lines
with context injected deterministically, because retrieval can miss them in a
40k-line log.

`looksLikeDebugPage` recognizes, one fixture-test per family
(`lib/__tests__/debug-page-detection.test.ts`):

- **CI**: Azure Pipelines (`##[error]`), GitHub Actions (`::error`), GitLab CI
  (`section_start`, `ERROR: Job failed`), Jenkins (`[Pipeline]`,
  `Finished: FAILURE`), CircleCI (`Exited with code`), docker build
  (`executor failed running`).
- **Test runners**: vitest/jest summaries, pytest (`FAILURES`, `E  assert`),
  go test (`--- FAIL:`).
- **Language crashes**: Python tracebacks (`File "…", line N`), Java/C# stacks,
  Go panics (`goroutine N [`), Rust panics, browser/node errors
  (`Uncaught TypeError`, `npm ERR!`).
- **Infra**: kernel/dmesg (`segfault at`, OOM-killer), k8s events
  (`CrashLoopBackOff`, `FailedScheduling`), systemd/journalctl
  (`status=1/FAILURE`).

Performance contract (also pinned by tests): single-hit crash markers run one
regex over the full text; the density heuristics run on a **sampled pass** —
first 500 + last 200 lines. The tail matters: long CI logs put the verdict at
the END, and a head-only sample misses exactly the pages this detector exists
for. Multi-MB logs stay under ~15 ms per call on the chat hot path.

Precision guard: pages *about* errors (error-handling docs, changelogs,
READMEs, news) must stay on the reading prompt — the negative fixtures pin
this. When extending the marker list, add the family fixture first.
