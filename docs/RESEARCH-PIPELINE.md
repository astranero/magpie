...
**Language rules**: the whole report is written in the same language as the
resolved research topic — auto-detected from the topic text itself via a
"REPORT LANGUAGE" instruction block prepended to every research LLM call (not
a stored user setting). Search queries are generated bilingually (original
language + English) to maximize discovery across sources.
...
without rewriting finished sections), scraped pages in `MagpieResearchCacheDB`. A 20 s heartbeat keeps the
