...
#### B1: Long-page context retrieval (P4-T4) ✅ (mechanism later superseded — see note)
...
> **Verify note:** this chunk+embed+cosine-rank path is dead code today —
> `PageChunkEmb`/`pageContextCache.chunks` are declared but never populated
> (`apps/extension/src/background/service-worker.ts:796-819`). It was
> replaced by an agentic tool loop (`agenticGather`, same file ~L816-2380)
> that gives the model `read_section`/`search_page`/`read_lines` tools over a
> head+section-index instead of pre-ranking chunks. The capability (long
> transcripts answered mid-video) still ships — just via the newer mechanism.
...
#### B3: Stream research synthesis (P4-T5) 🟡 live-render bubble later reverted
...
> **Verify note:** shipped as described, then deliberately reverted
> ("Stop live-rendering the research report — fixes frozen caret / dead
> panel"): re-parsing the whole growing markdown report on every ~150ms delta
> saturated the main thread. `DEEP_RESEARCH_DELTA` still broadcasts from the
> SSE helper (`service-worker.ts:3436`) and the persisted report still
> replaces the live view via `loadChatHistory` on DONE, but the client now
> intentionally drops every delta (`App.tsx` `DEEP_RESEARCH_DELTA` handler,
> ~L621) — the field log shows progress text instead of live report tokens.
...
#### D2: GitHub Actions CI ✅ (.github/workflows/verify.yml)
...
- **E1** ✅ MV3 persistence blog post — `docs/MV3-PERSISTENT-AGENT-STATE.md`
  is now the full post (intro, problem, 5-pillar pattern, the 5-min
  resume-loop war story, code pointers, conclusion), not just a skeleton. S
