...
CI (`.github/workflows/verify.yml`) runs all three on push/PR, workspace-aware.
...
- `smoke.spec.ts` — sidepanel mounts, bottom-nav switches views, chat empty
  state, command palette, and that the BYOK model picker searches without
  touching Copilot's config (the two provider sections stay independent).
...
- `citation-chips.spec.ts` — harvested-anchor and `RESOLVE_CITATIONS`
  integrity against the real chunk store, plus UI click-through: a `#cite:`
  chip in a saved report opens the source document at the exact cited
  passage (never a different doc, never the external URL).
...
- Live-provider E2E (`e2e/live-*.spec.ts`): real OpenRouter runs — chat
  streaming + process indication, /deepresearch plan card, /teach lesson
  persistence + /grill one-at-a-time pacing, vision import, bad-key
  negative; full research run behind `RUN_LIVE_RESEARCH=1`. Skips
