...
- [x] Onboarding: BYOK (OpenAI-compatible endpoint) or GitHub Copilot SSO.
      ...
      hook below has been re-messaged to "Ollama support" (manual BYOK base
...
- [ ] Repo hygiene: LICENSE and CONTRIBUTING.md (both present), issue
      templates still missing; `SECURITY.md` already strong — link it
      prominently (trust signal).
...
1. **r/LocalLLaMA** — the highest-affinity audience on the internet for
   "local-first + BYO model + Ollama support". Post as a build story, not an
